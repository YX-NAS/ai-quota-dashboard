...
![套餐实时额度](docs/screenshots/quotas.png)

**每天的钱花在哪、哪个模型最烧钱**，一张明细表 + 一张趋势图说清楚：

![每日明细](docs/screenshots/daily.png)

**所有东西都能改**：汇率、月额度、模型单价、额度 API Key（脱敏回显），网页里点开设置就行：

![设置面板](docs/screenshots/settings.png)

> 📷 以上截图均为**演示数据**，由假数据生成，不含任何真实用量。
